...
**Cómo importar, usar y entender `WHSolver.py`**

El archivo `WHSolver.py` funciona como una pequeña librería personal para resolver sistemas lineales y sistemas no lineales desde notebooks de Python.

```mermaid
flowchart LR
    A["WHSolver.py<br/>archivo módulo"] --> B["LinearSystem<br/>sistemas Ax=b"]
...
```mermaid
flowchart TB
    M["Módulo<br/><code>WHSolver.py</code>"]
...
Si editas `WHSolver.py` y el notebook no reconoce los cambios, reinicia el kernel o recarga el módulo:
...
|---|---|---|
| `ModuleNotFoundError` | El notebook no ve `WHSolver.py`. | Coloca el archivo en la misma carpeta o usa `sys.path.append`. |
| `from WHSolver.py import *` falla | Se incluyó la extensión `.py`. | Usa `from WHSolver import *`. |
...
```mermaid
flowchart LR
    A["No<br/>from WHSolver.py import *"] --> B["Sí<br/>from WHSolver import *"]
```
...
```mermaid
flowchart TB
    A["1. WHSolver.py está en la carpeta correcta"] --> B["2. Importaste con<br/>from WHSolver import *"]
...
```mermaid
flowchart LR
    A["Importar<br/>from WHSolver import *"] --> B["Elegir<br/>lineal o no lineal"]
